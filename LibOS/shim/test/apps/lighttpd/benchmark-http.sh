...
do
	...
	do
		rm -f OUTPUT
		echo "ab $OPTIONS -n $REQUESTS -c $CONCURRENCY http://$DOWNLOAD_HOST:$PORT/$DOWNLOAD_FILE"
		ab $OPTIONS -n $REQUESTS -c $CONCURRENCY http://$DOWNLOAD_HOST:$PORT/$DOWNLOAD_FILE > OUTPUT || exit $?
